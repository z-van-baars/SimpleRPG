...
func _on_Skeleton_death():
	skeleton_count = max(0, skeleton_count - 1)


func test_position(position: Vector2):
	# Check if one of the cell types in this position is grass or sand
	var cell_coord = tilemap.world_to_map(position)
	var cell_type_id = tilemap.get_cellv(cell_coord)
	var grass_or_sand = (cell_type_id == tilemap.tile_set.find_tile_by_name("Grass")) || (cell_type_id ==
	tilemap.tile_set.find_tile_by_name("Sand"))
